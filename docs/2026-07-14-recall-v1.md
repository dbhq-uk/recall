...
- **Prior art is fair game; our code stays our own.** Study whatever helps — other retrieval tools (including `claude-context` and forks), vendor docs, papers — and borrow ideas freely. Do not paste another project's source verbatim: recall's own implementation is written here so the repo stays cleanly MIT-licensable. For mechanism, still prefer the primary source (pgvector docs, Postgres FTS docs, ParadeDB pg_search docs, MCP Python SDK, Ollama API docs, RRF paper) — it is more reliable than reading it out of a codebase.
...
- Provenance (design §Provenance, carried over): built on published IR literature and vendor docs, informed by prior art in the space, own code original and MIT-licensed.
